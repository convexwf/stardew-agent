...
持续模式不是普通的“一次执行后结束”队列步骤，而是由 Mod 在游戏线程中维护的长期任务状态机。Runtime 负责启动、停止、actor lease、目标约束和结果对账；Mod 负责逐 Tick 读取游戏状态、选择目标、移动、调用游戏 API 和清理动作。

一个 `companion-1` 同时只能运行一个持续模式或 foreground action。模式启动后返回异步 request ID，在 `paused` 或 `waiting` 期间保留活动状态；Runtime 通过状态快照和后续事件决定恢复、重规划或取消。具体的模式清单、游戏对象约束、工具/背包前置条件和 Bubble 模板见 [Demo 持续模式扩展](../demo/cli-file-bridge.md#持续模式扩展)。当前 `follow` 的具体寻路、跨地图 warp 和取消行为仍以 [Demo Follow 设计](../demo/cli-file-bridge.md#follow-设计与实现) 为准。
