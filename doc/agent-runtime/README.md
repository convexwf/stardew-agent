...
持续模式不是由 Runtime 反复提交低级 `move_to` 和 `use_tool` 组成的脚本，而是由 Mod 在游戏线程中维护的长期任务状态机。Runtime 负责启动、停止、租约、目标约束和结果对账；Mod 负责逐 Tick 读取游戏状态、选择目标、移动、调用游戏 API 和清理动作。

每个持续模式都持有一个 actor lease。一个 `companion-1` 同时只能运行一个持续模式或 foreground action；模式启动后返回异步 request ID，直到完成、失败、取消或被新的计划取代才写入终态结果。模式在 `paused` 或 `waiting` 期间保留活动状态，Runtime 通过状态快照和后续事件决定恢复、重规划或取消。
...
| `plant_crops` | 在已开垦且符合条件的耕地上播种 | 没有可用种子、没有符合条件的已开垦土地、背包或路径问题、用户取消 |
| `mine` | 在矿井中处理矿石、梯子和必要的战斗 | 矿井目标完成、工具或资源不足、地图/战斗前置条件失败、用户取消 |
| `fish` | 在钓鱼地点执行鱼竿状态机 | 钓鱼目标完成、没有鱼竿、鱼竿状态失败、用户取消 |

这些模式共享目标选择、路径任务、工具/物品前置检查、失败重试、气泡通知和取消清理机制；每个模式只实现自己的目标扫描和成功条件。当前 `follow` 的具体寻路、跨地图 warp 和取消行为仍以 [Demo 技术实现方案](../demo/cli-file-bridge.md) 为准。

#### `plant_crops` 的土地约束

`plant_crops` 只允许在已经存在的已开垦土地上播种。模式选择目标时必须从当前 `GameLocation` 的耕地数据中筛选已存在的 `HoeDirt`，并满足以下条件：

1. tile 已经是 `HoeDirt`，不是普通地面、草地或其他可通行 tile；
2. 该 tile 没有现有作物，且游戏 API 判断当前种子可以种植；
3. Shadow Farmer 背包中存在可用种子，并满足季节、地点和种子本身的游戏前置条件；
4. Companion 能够到达该 tile，播种后重新读取背包和耕地状态确认结果。

该模式不得调用锄地逻辑，不得自动把普通地面转换成 `HoeDirt`，也不得为了扩张种植区域而改变土地状态。没有符合条件的已开垦土地时，模式返回正常完成或受约束的暂停结果，不能把“未开垦土地”当成播种失败后自动修复的目标。

播种一次只消耗一个种子。若游戏 API 调用成功但背包堆叠或耕地作物状态没有按预期变化，Runtime 将该步骤标记为 `uncertain`，先重新观察；连续验证失败才进入重规划，不继续盲目消耗种子。

#### 模式阻塞与气泡通知

模式遇到可恢复问题时进入 `paused` 或 `waiting`，遇到不可恢复问题时进入 `failed`，并通过官方 NPC 气泡接口向玩家提示。气泡是模式内部的通知，不另建一个外部 `bubble` action，也不抢占当前 actor lease。通知应带原因和冷却时间，避免每个 Tick 重复显示。

Mod 配置可以提供可替换的文本模板，模板只负责展示，不改变模式状态机：
...
- `plant_crops` 只选择已存在的 `HoeDirt`，不会对普通地面调用锄地或创建耕地；
- 播种后的种子堆叠、耕地作物状态和无可用目标结果能够被验证；
- 缺少种子、没有已开垦土地、路径阻塞和低体力时的暂停、气泡通知与恢复；
