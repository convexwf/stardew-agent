...
`chop_trees` 的目标是持续寻找可砍伐的树木，并使用 Shadow Farmer 背包中的斧头完成砍伐。它不是对地图上所有可交互对象进行尝试，而是先通过目标扫描器识别允许处理的树木，再为单个目标建立移动和工具动作。

单个目标的处理顺序为：检查斧头和世界状态 → 选择可达树木 → 移动到树木附近 → 调整朝向并调用斧头 API → 重新扫描树木和掉落物状态 → 选择下一个目标。验证至少要确认树木状态发生变化或树木被砍倒，并记录工具使用结果；不能仅凭调用 API 没有抛异常就认为目标完成。

目标扫描需要排除不可砍伐对象、建筑、作物和不应由该模式处理的特殊树木。允许的树木类型和地点应由模式参数或策略限定，不能默认把所有 `Tree` 实例都当作安全目标。没有符合条件的树木时正常完成；没有斧头时进入 `paused` 并提示；路径连续失败时重算有限次数，超过上限后暂停或失败。

### `water_crops`

`water_crops` 只处理已经存在的耕地数据中的作物，不负责锄地、播种或收获。目标扫描选择需要浇水的 `HoeDirt`，并排除没有作物、已经完成当日浇水或当前天气和游戏规则不需要浇水的 tile。

单个目标的处理顺序为：检查浇水壶和剩余水量 → 选择需要浇水且可到达的耕地 → 移动到有效操作位置 → 朝向目标并调用浇水 API → 重新读取耕地的浇水状态。只有目标状态确认变为已浇水，才计入完成数量；工具调用失败、目标被占用或状态没有变化时进入重试或 `uncertain`，不能继续无条件遍历。

当日没有需要浇水的作物时正常完成。缺少浇水壶、浇水壶为空、体力不足、地点不允许操作或路径持续阻塞时暂停并提示。该模式不应为了寻找更多目标自动切换到未授权的地图，也不应把普通地面当成耕地目标。
...
3. **处理目标**：移动到目标附近，使用镐破坏矿石或调用近战动作处理威胁，每次动作后重新观察；
4. **推进楼层**：发现并确认梯子或出口后，按目标决定停留、下降或返回；地图切换后重新同步可见 NPC 和 Shadow Farmer；
5. **安全结束**：达到目标楼层、目标数量、时间、体力或生命阈值时停止，并写出可验证的终态信息。

矿井模式需要同时检查镐、近战武器、生命、体力和背包容量。没有工具时暂停；没有武器但遇到必须处理的怪物时暂停或失败；生命、体力或游戏时间达到安全阈值时主动结束，不应为了继续执行而强行战斗。目标不可达、楼层加载失败、战斗结果无法确认或跨地图同步失败时，进入 `paused`/`failed` 并通过 Bubble 说明原因。
...
每个模式都需要分别验证目标扫描、路径阻塞、工具或物品前置条件、游戏副作用、结果确认、气泡冷却、暂停恢复和取消。各模式的额外验证重点如下：

| 模式 | 额外验证重点 |
| --- | --- |
| `chop_trees` | 普通树木、不可处理对象、斧头消耗、树木状态变化、掉落物和连续目标切换 |
| `water_crops` | 需要浇水和无需浇水的耕地、空耕地、浇水壶余量、天气影响和浇水状态变化 |
