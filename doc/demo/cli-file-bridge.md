...
每个模式都需要分别验证目标扫描、路径阻塞、工具或物品前置条件、游戏副作用、结果确认、气泡冷却、暂停恢复和取消。`plant_crops` 额外验证普通地面、已经开垦的空地、已有作物的耕地、不可种植季节、没有种子和种子堆叠扣减；Fake Mod 只能验证状态机和协议，土地类型及播种效果仍需 Windows + SMAPI 实机验证。
