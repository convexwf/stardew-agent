...
internal sealed class ModEntry : Mod
{
    ...
    public override void Entry(IModHelper helper)
    {
        ...
        _moveExecutor = new MoveExecutor(helper);

        helper.Events.GameLoop.UpdateTicked += OnUpdateTicked;
        helper.Events.GameLoop.OneSecondUpdateTicked += OnOneSecondUpdateTicked;
        helper.Events.GameLoop.ReturnedToTitle += OnReturnedToTitle;

        Monitor.Log($"Loaded. Bridge directory: {paths.Root}", LogLevel.Info);
    }
    ...
    private void WriteSnapshot()
    {
        ...
        var snapshot = Protocol.CreateEnvelope("snapshot", null, payload);
        _store.WriteJson(Path.Combine(_store.Paths.Snapshots, $"snapshot-{payload.Sequence}.json"), snapshot);
    }
